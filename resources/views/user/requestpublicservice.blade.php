...
@endsection
@section("title")
{{__('request.request public service')}}
    ...
    <div class="container">
        <div class="form px-3">
            ...
            <div class="section-header">
                <h2> {{__('request.request service')}}</h2>
            </div>
            ...
            <form action="{{ route('user.request.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control w-100 @error('description') is-invalid @enderror"
                        rows="3" placeholder="Descripe"></textarea>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date"
                        name="due_date" min="{{date_format(now(),'Y-m-d')}}">
                    @error('due_date')<div class="alert alert-danger">{{ $message }}</div>@enderror
                </div>
                <input type="hidden" name="type" value="public">
                <button type="submit"
                    class="btn btn-modal my-3 px-5 btn-model-primary request-button ">{{__('request.request')}}</button>
            </form>
        </div>
    </div>
</div>
