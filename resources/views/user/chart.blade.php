...
<div class="charts">
  <div class="container-fluid">
    <div class="section-header ">
      <h2> {{__('translate.shopping cart')}} </h2>
    </div>
    ...
    <div class=" chart-container d-flex flex-column flex-md-row ">
      <div class="chartsitems ">
        ...
        <div class="chartitem row justify-content-center align-items-center g-2">
          ...
          <div class="col">
            ...
            <p>{{$cartitems->price}}<span>{{__('translate.S.R')}}</span></p>
          </div>
          <div class="col">
      ...
      <div class="chartcheckout px-2">
        <h3 class="py-2 px-3 ">{{__('translate.order summery')}}</h3>
        <div class="d-flex justify-content-center ">
          @if ($descount==0)
          <form action="{{route('user.addPromoCode')}}" class="copone px-lg-3">
            @csrf
            <div class="d-flex justify-content-center">
              <input type="text" class="form-control" id="promo" placeholder="{{__('translate.promo code')}}"
                name="code">
              <button type="submit" class="btn py-2 px-lg-4 ms-3"> {{__('translate.apply')}} </button>
            </div>
          </form>

          @else

          <div class="text-center   w-100 py-2 text-success">{{__('translate.promo code is actived')}}</div>
          @endif

        </div>
        <div class="details mt-4">
          <div class="price d-flex">
            <p class="fs-4">{{__('translate.price')}}</p>
            <div class="number ">
              {{$price}}
              <span>{{__('translate.S.R')}}</span>
            </div>
          </div>
          <div class="price d-flex ">
            <p class="fs-4">{{__('translate.discount')}}</p>
            <div class="number">
              {{$descount}}
              <span>%</span>
            </div>
          ...
          <div class="price d-flex">
            <p class="fs-4">{{__('translate.total')}}</p>
            <div class="number">
              {{$total}}
              <span>{{__('translate.S.R')}}</span>
            </div>
          </div>
          <button type="button" class="btn  btn-modal  my-3 btn-model-primary chart-out" data-bs-target="#pay"
            id='paygetid' data-bs-toggle="modal">{{__('translate.checkout')}}</button>
        </div>
      </div>
...
<div class="modal offers fade  modal-uk pay" id="pay" aria-hidden="true" aria-labelledby="exampleModalToggleLabel"
  tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <h1> {{__('translate.payment method')}}</h1>
