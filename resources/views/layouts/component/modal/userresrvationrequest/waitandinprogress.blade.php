<div id="userreservationwaitandinprogress{{ $request->id }}" class="modal offers fade" aria-hidden="true"
            ...
            <div class="modal-body">
                <div class="div d-flex justify-content-start px-4">
                    <div class="d-flex flex-column">
                        ...
                        @if( $request->status == 'Waiting' && $request->date_time==now()->toDateString() &&
                        ($request->from<=now() ||$request->to <=now())) <span class="inprogress">
                                {{__('request.In Process')}}</span>
                ...
                <div class="btn-contianer d-flex flex-column justify-between align-items-center my-3">
                    <p class="craz-color fw-light" style="font-size: 14px;">
                        {{__('translate.You can cancel your reservation before 48 hours from reserve date')}}</p>
                    <button class=" btn-cormoz btn-modal border-0" type="button" data-bs-toggle="modal"
                        data-bs-target="#surdeletereservation{{$request->id}}"> {{__('translate.cancel')}}</button>
                </div>
                @endif
            </div>
        </div>
