<div id="userReservationPendingAceptOrReject{{ $request->id }}" class="modal offers fade" aria-hidden="true"
            ...
            <div class="modal-body">
                <div class="div d-flex justify-content-start px-4">
                    <div class="d-flex flex-column">
                        ...
                        <span class="text-black-50">{{__('request.'.$request->status)}}</span>
                    </div>
                ...
                <div
                    class="btn-contianer d-flex flex-lg-row flex-column-reverse justify-content-center align-items-center my-3  gap-2 gap-lg-0">
                    <button class=" btn-reject border-0 btn-modal rounded-pill mx-2" type="button"
                        data-bs-toggle="modal"
                        data-bs-target="#userrejectreservationoffer{{$request->id}}">{{__('request.reject')}}
                    </button>
                    <button class=" btn-accept  paybutton border-0 btn-modal rounded-pill mx-2" type="button"
                        data-bs-toggle="modal" data-bs-target="#pay{{$request->id}}" data-res='{{$request->id}}'>
                        {{__('request.accept')}}</button>
                </div>
            </div>
        </div>
    </div>
