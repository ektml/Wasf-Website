<div class="modal offers fade" id="feelancerrequstPendingcancancel{{$request->id}}" tabindex="-1"
...
">
      ...
      <div class="px-2">
        <h5 class="text-black border-top mt-2 pt-2 "> {{__('request.Total price')}}</h5>
        <div class="d-flex justify-content-between">
          ...
          <p class="fw-900 mb-0 text-black">
            <button data-bs-toggle="modal" type="button" data-bs-target="#sendofferforrequest{{$request->id}}" style="
       border-radius: 50%;
       padding: 1px 5px;
       background-color: #fff;
       border: none; "><i class="fa-solid fa-pen fa-bounce" style="color: #eb3d1e;"></i></button>
            {{$request->offer->where('freelancer_id',auth()->user()->id)->first()->price}}

            <span class="text-black-50 mx-1"> {{__('translate.SR')}}</span>
          </p>
          @endif
        </div>
      </div>
...
<div style="position:fixed ; bottom:0;right:0; font-size:30px">
  <button class="addrequesticon" type="button" data-bs-toggle="offcanvas"
   data-bs-target="#chat{{$request->id}}"
