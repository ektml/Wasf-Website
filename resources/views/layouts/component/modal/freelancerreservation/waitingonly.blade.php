            ...
            <div class="d-flex flex-column px-5">
                ...
                <div class="btn-contianer d-flex flex-column justify-content-center align-items-center my-3">
                    ...
                    <form action="{{route('freelanc.reservation.finish',$request->id)}}" method="GET">
                        <button class="btn-modal  my-3 btn-model-primary border-0"type="submit">{{ __('request.Finish')}}</button>
                    ...
                    <button class="btn-cormoz btn-modal border-0"type="button" data-bs-toggle="modal" data-bs-target="#requestdelay{{$request->id}}" >{{ __("request.Request Delay")}}</button>
                    @endif

                    @if( now()->addDay(5)->toDateString() <= $request->date_time)
                    <button class="btn-cormoz btn-modal border-0"type="button" data-bs-toggle="modal" data-bs-target="#suredelete{{$request->id}}" >{{ __('request.cancel')}}</button>
