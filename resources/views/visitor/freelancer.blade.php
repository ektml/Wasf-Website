        ...
        <section class="freelanc v2  freelancerinprofile" style="max-width: 1200px;">
            ...
            <div class="totals">
                <div class="projects">
                    ...
                    <p>{{  App\Models\Requests::where('freelancer_id', $freelancer->id)->where('status','Completed')->count() }}<sub>{{__('freelancerprofile.projects')}}</sub>
                    </p>
                </div>

                <div class="productstotal">
                    <i class="fa-solid fa-dollar-sign"></i>
                    <p>{{ App\Models\Product::where('freelancer_id', $freelancer->id)->count() }}<sub>{{__('freelancerprofile.products')}}</sub>
                    </p>
                </div>

                <div class="photos">
                    <i class="fa-solid fa-image"></i>
                    <p>{{ App\Models\Photo::where('freelancer_id', $freelancer->id)->count() }}<sub>{{__('freelancerprofile.photos')}}</sub>
                    </p>
                </div>
            </div>
        </section>
            ...
            <div class="card" style="min-width:290px">
                <div class="image-product">
                    <a href="{{route('photo',$photo->id)}}">
                        <img src="{{ asset('assets/images/photo/'.$photo->photo) }}" class="card-img-top"
                            alt="Photo"></a>

                    @auth
                    <button type="button" data-type="photo" data-id="{{$photo->id}}" onclick="likes(this)"
                    ...
                    <button class="addtochart active" data-id="{{$photo->id}}" onclick="addcart(this)"
                        data-type='photo'> {{__('translate.in cart')}}</button>
                    @else
                    <button class="addtochart" data-id="{{$photo->id}}" onclick="addcart(this)"
...
<!-- chooseservice -->
<div id="chooseservice" class="modal fade p-5  modal-uk" aria-hidden="true" aria-labelledby="chooseserviceLabel2"
    tabindex="-1">
